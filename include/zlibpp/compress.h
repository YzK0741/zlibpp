...
namespace zlibpp {
    ...
    inline std::size_t compress_bound(const std::size_t size) noexcept {
        return ::compressBound(size);
    }

    template<typename SrcT, typename DesT>
    [[nodiscard]] std::expected<unsigned long, err>
    compress(std::span<SrcT> source, std::span<DesT> dest) noexcept {
    ...
    [[nodiscard]] std::expected<unsigned long, err>
    compress (source_container&& source, dest_container&& dest) noexcept {
    ...
    compress(
        ...
        if (source == nullptr || source_size < 1) {
            return std::unexpected(err::data_error);
        }
        ...
        return compress(std::span(source, source_size), std::span(dest, dest_size));
    }
    ...
    compress(
        ...
        return compress(source.get(), destination.get(), source_size, dest_len);
    }
    ...
    [[nodiscard]] std::expected<unsigned long, err>
    compress(
        const std::weak_ptr<SrcT>& source,
        const std::weak_ptr<DesT>& dest,
        std::size_t source_len,
        std::size_t dest_len = 0
        ) noexcept {
