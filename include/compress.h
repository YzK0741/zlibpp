...
namespace zlibpp {
    ...
    compress(
        ...
        if (source == nullptr || dest == nullptr || source_len < 1) {
            return std::nullopt;
        }
    ...
    compress(std::span<const T> source) {
        ...
        if (auto compress_result = compress(source, std::span(result_temp.get(), expected_size)); compress_result) {
            auto result = std::make_unique<unsigned char[]>(*compress_result);
            memcpy(result.get(), result_temp.get(), *compress_result);
            return result;
        } else {
            return std::nullopt;
        }
    ...
    [[nodiscard]] std::optional<std::unique_ptr<unsigned char[]>>
    compress(const container&& source) {
